...
def apply_fill_to_session_ledger(
    ...
    is_exit = (action_type or "").upper() in EXIT_ACTIONS
    with lock:
        ledger = getattr(trader, "_session_open_qty", None)
        if ledger is None:
            return
        ...
        if is_exit:
            new_qty = max(0, current - fill_qty)
        else:
            new_qty = current + fill_qty
        if new_qty <= 0:
            ledger.pop(sym, None)
        else:
            ledger[sym] = new_qty
...
def build_eod_exit_plan(
    ...
    if eod_use_session_ledger_enabled():
        ...
        for sym in sorted(symbols_to_check):
            ...
            exit_qty = min(ledger_qty, broker_qty)
            if exit_qty <= 0:
                continue

            side = (broker_pos or {}).get("side", "BUY")
            plan.append({
                "symbol": sym,
                "side": side,
                "qty": exit_qty,
            ...
            print(
                ...
                f"broker_net={broker_qty} exit_qty={exit_qty}"
            )
